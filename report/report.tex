...
图 \ref{fig:mesh_convergence} 展示了使用 Quad4 单元在不同网格密度下的挠度分布。可以看出，随着网格加密，挠度场变得更加平滑，且中心最大挠度值逐渐收敛。粗网格 ($5 \times 5$) 虽然能捕捉到整体变形趋势，但在极值点和边缘处的精度较低。

\begin{figure}[H]
    \centering
    \begin{minipage}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../result/problem2/Quad4_5x5/1_Deflection_w.png}
        \caption{Quad4 $5 \times 5$}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../result/problem2/Quad4_10x10/1_Deflection_w.png}
        \caption{Quad4 $10 \times 10$}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../result/problem2/Quad4_50x50/1_Deflection_w.png}
        \caption{Quad4 $50 \times 50$}
    \end{minipage}
    \caption{不同网格密度下的挠度云图对比 (Quad4)}
    \label{fig:mesh_convergence}
\end{figure}

同样地，图 \ref{fig:mesh_convergence_tri3} 展示了使用 Tri3 单元在不同网格密度下的挠度分布。

\begin{figure}[H]
    \centering
    \begin{minipage}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../result/problem2/Tri3_5x5/1_Deflection_w.png}
        \caption{Tri3 $5 \times 5$}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../result/problem2/Tri3_10x10/1_Deflection_w.png}
        \caption{Tri3 $10 \times 10$}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../result/problem2/Tri3_50x50/1_Deflection_w.png}
        \caption{Tri3 $50 \times 50$}
    \end{minipage}
    \caption{不同网格密度下的挠度云图对比 (Tri3)}
    \label{fig:mesh_convergence_tri3}
\end{figure}

综合对比 Tri3 和 Quad4 两种单元的结果表明，随着网格密度的增加，中心挠度均收敛于一个稳定值。在相同节点数下，Quad4 单元通常比 Tri3 单元表现出更快的收敛速度和更高的精度。
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|c|}
        \hline
        \textbf{指标} & \textbf{MATLAB (Quad4)} & \textbf{Abaqus (S4R)} & \textbf{相对误差 (\%)} \\
        \hline
        中心挠度 $w_{max}$ (m) & -1.088290e-03 &  -1.08737e-03 & 0.085\% \\
        \hline
        最大 Von Mises 应力 (Pa) &   1.026135e+08  & 1.02514e+08 & 0.097\% \\
        \hline
    \end{tabular}
\end{table}
...
总体上看计算结果与 abaqus 高度一致，但仍存在一定差异，可能原因包括：
\begin{enumerate}
